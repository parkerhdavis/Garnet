...
import type { PreviewPanelHandle } from "@/plugins/texturing/types/pbr";

type TilingMode = "repeat" | "mirror";

const TilingPreviewPanel = forwardRef<PreviewPanelHandle>(
	function TilingPreviewPanel(_props, ref) {
		...
		useEffect(() => {
			...
			img.src = inputPreview.startsWith("data:")
				? inputPreview
				: `data:image/png;base64,${inputPreview}`;
		}, [inputPreview]);
			...
			<div className="flex flex-1 min-w-0">
				...
				<div className="w-72 shrink-0 flex flex-col border-r border-base-300 overflow-y-auto">
					...
					<div className="px-3 pt-3 pb-2 border-b border-base-300 shrink-0">
						...
						<div className="text-xs text-base-content/40 mt-0.5 leading-snug">
							Tile a texture in a grid to visually check seamlessness. Use
							offset mode to inspect center seams.
						</div>
					</div>
				...
				<div className="flex-1 min-w-0 flex flex-col">
					...
					<div
						...
						style={{
							backgroundImage:
								"linear-gradient(45deg, var(--color-base-300) 25%, transparent 25%, transparent 75%, var(--color-base-300) 75%), linear-gradient(45deg, var(--color-base-300) 25%, transparent 25%, transparent 75%, var(--color-base-300) 75%)",
